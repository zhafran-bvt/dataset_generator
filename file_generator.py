    ...
    for i in range(len(points)):
        ...
        try:
            ...
            if (
                ...
                if (
                    center_lon < lon_min
                    or center_lon > lon_max
                    or center_lat < lat_min
                    or center_lat > lat_max
                ):
                    logger.debug(
                        f"Filtered H3 cell {h3_cell} with center ({center_lon:.6f}, {center_lat:.6f}) outside bounding box"
                    )
                    continue
            ...
            if land_polys:
                ...
                if not point_in_land:
                    logger.debug(
                        f"Filtered H3 cell {h3_cell} with center ({center_lon:.6f}, {center_lat:.6f}) outside land boundaries"
                    )
                    continue
            ...
            logger.warning(
                f"Failed to convert point ({lon:.6f}, {lat:.6f}) to H3 cell: {e}"
            )
            continue
...
class CorrelationEngine:
    ...
    def generate_batch_values(self, labels, batch_size=10000, seed=None):
        if seed is not None:
            np.random.seed(seed)
        values = {
            label: np.random.random(batch_size)
            for label in labels
            if label in self.value_ranges
        }
        for _ in range(3):
...
def random_sample_in_geometry(
    n, polygons, lon_min, lon_max, lat_min, lat_max, batch_size=1000
):
    ...
    while len(points) < n and attempts < max_attempts:
        ...
        candidates = get_random_points_in_bbox(
            ...
            lat_min,
            lat_max,
            min(max(batch_size, want * 5), want * 20),
        )
    ...
    if geom_type == "H3":
        ...
        for i in range(n):
            ...
            for attempt in range(max_attempts):
                lon, lat = points[i]
                logger.debug(
                    f"Attempt {attempt} for index {i}: lon={lon:.6f}, lat={lat:.6f}"
                )
                # Size range with progressive reduction and minimum threshold
                size_factor = (
                    max(0.0025, 0.025 * (1.0 / (attempt // 20 + 1)))
                    if attempt >= 20
                    else random.uniform(0.005, 0.05)
                )
                ...
                if not clipped_geom.is_valid or clipped_geom.is_empty:
                    adjustment_factor = (
                        min(0.1, 0.01 * (attempt // 50 + 1)) * lon_extent
                    )
                ...
                if isinstance(clipped_geom, MultiPolygon):
                    ...
                    if best_poly:
                        ...
                        adjustment = random.uniform(0.001, adjustment_factor)
                        lon += (
                            adjustment if random.choice([True, False]) else -adjustment
                        )
                        lat += (
                            adjustment if random.choice([True, False]) else -adjustment
                        )
                ...
                for x, y in coords_to_check[:-1]:
                    if not any(
                        Point(x, y).within(Polygon(poly)) for poly in land_polys
                    ):
                        all_within = False
                        break
                if not all_within:
                ...
                if tree:
                    ...
                    if any(
                        new_multi.intersects(
                            loads(g) if format_type == "WKT" else shape(json.loads(g))
                        )
                        for g in geoms
                    ):
                        adjustment_factor = (
            ...
            if last_valid_coords is None:
                logger.error(
                    f"Failed to generate non-overlapping {geom_type} at index {i} after {max_attempts} attempts (including fallback). Skipping index {i}."
                )
                continue  # Skip to the next index instead of raising an exception
            if format_type == "WKT":
                coord_str = ", ".join(
                    [f"{x:.6f} {y:.6f}" for x, y in last_valid_coords]
                )
                ...
                if geom_type == "POLYGON":
                    geoms.append(
                        json.dumps(
                            {"type": "Polygon", "coordinates": [last_valid_coords]}
                        )
                    )
                else:  # MULTIPOLYGON
                    geoms.append(
...
def create_spatial_clustering_model(
    ...
    if land_geometry:
        ...
        centers = random_sample_in_geometry(
            cluster_count, polygons, lon_min, lon_max, lat_min, lat_max
        )
    else:
        centers = get_random_points_in_bbox(
            lon_min, lon_max, lat_min, lat_max, cluster_count
        )
    samples = []
    ...
    return {"model": kde_model, "use_probability": 0.85, "centers": centers}
...
def generate_points_for_batch(params):
    ...
    if (
        use_spatial_clustering
        and spatial_clusters
        and random.random() < spatial_clusters["use_probability"]
    ):
                ...
                if not is_valid:
                    ...
                    for _ in range(10):
                        valid_points = random_sample_in_geometry(
                            1, polygons, lon_min, lon_max, lat_min, lat_max
                        )
                    ...
                    if not replaced:
                        ...
                        valid_points = random_sample_in_geometry(
                            50, polygons, lon_min, lon_max, lat_min, lat_max
                        )
                        if valid_points.shape[0] >= 1:
                            points[i] = valid_points[0]
    elif polygons:
        points = random_sample_in_geometry(
            batch_size, polygons, lon_min, lon_max, lat_min, lat_max
        )
    else:
...
def generate_batch_data(batch_params):
    ...
    correlated_values = correlation_engine.generate_batch_values(
        labels, batch_size, seed=start_id
    )
    ...
    for i in range(batch_size):
        if i < len(
            geoms
        ):  # Only include rows where geometries were successfully generated
            ...
            if geom_type == "H3" and H3_AVAILABLE:
                try:
                    ...
                    logger.warning(
                        f"Failed to extract coordinates from H3 cell {geoms[i]}: {e}"
                    )
                    row["latitude"] = None
                    row["longitude"] = None
            ...
            for label in labels:
                if label not in row and label not in [
                    "id",
                    "geom",
                    "date_created",
                    "latitude",
...
def save_files_chunked(df, filename_prefix, chunk_size=100000):
    ...
    if total_rows <= chunk_size:
        ...
        logger.info(
            f"Saved: {csv_file} ({os.path.getsize(csv_file) / 1024 / 1024:.2f} MB)"
        )
        try:
            if total_rows <= 1000000:
                ...
                logger.info(
                    f"Saved: {excel_file} ({os.path.getsize(excel_file) / 1024 / 1024:.2f} MB)"
                )
            else:
                logger.warning(
                    "Dataset too large for Excel (> 1M rows). Excel file not created."
                )
        except Exception as e:
        ...
        for i in range(num_chunks):
            ...
            logger.info(
                f"Saved chunk {i+1}/{num_chunks}: {chunk_file} ({os.path.getsize(chunk_file) / 1024 / 1024:.2f} MB)"
            )
        logger.info(
            f"CSV data saved in {num_chunks} chunks in the '{output_dir}' directory."
        )
...
def analyze_correlations(df, sample_size=10000):
    ...
    for var1, var2, target_corr in VARIABLE_CORRELATIONS:
        if var1 in corr_matrix.columns and var2 in corr_matrix.columns:
            ...
            logger.info(
                f"{var1} vs {var2:<20} | {target_corr:8.2f} | {actual_corr:8.2f}"
            )
    logger.info("-" * 50)
...
def generate_parallel_dataframe(
    ...
    available_labels = [
        label for label in REALISTIC_LABELS if label not in fixed_columns
    ]
    ...
    additional_labels = (
        random.sample(available_labels, min(num_additional, len(available_labels)))
        if num_additional > 0
        else []
    )
    ...
    if land_geometry:
        polygons = extract_polygon_coords(land_geometry)
        logger.info(
            f"Extracted {len(polygons)} polygon(s) for efficient point-in-polygon testing"
        )
    ...
    if use_spatial_clustering:
        ...
        logger.info(
            f"Created model with {len(spatial_clusters['centers'])} population clusters"
        )
    ...
    with tqdm(total=rows, desc="Generating data") as progress:
        with ProcessPoolExecutor(max_workers=num_cores) as executor:
            futures = [
                executor.submit(generate_batch_data, params) for params in batch_params
            ]
...
class DataValidator:
    ...
    def validate_statistical_properties(self, df, expected_correlations, value_ranges):
        ...
        for col in df.columns:
            if col in value_ranges and pd.api.types.is_numeric_dtype(df[col]):
                ...
                outliers = (
                    np.abs(z_scores) > self.validation_options["outlier_threshold"]
                )
                if outliers.any():
        ...
        if self.validation_options["generate_reports"]:
            self._generate_statistical_report(
                df, validation_results, expected_correlations
            )
    ...
    def validate_geometries(self, df, geom_column="geom", format_type="WKT"):
        ...
        bounds = None
        if (
            hasattr(self, "lon_min")
            and hasattr(self, "lon_max")
            and hasattr(self, "lat_min")
            and hasattr(self, "lat_max")
        ):
        ...
        for i, geom_str in enumerate(df[geom_column]):
            try:
                ...
                if geom.is_valid:
                    ...
                    if len(validation_results["invalid_samples"]) < 10:
                        validation_results["invalid_samples"].append(
                            {"index": i, "geometry": geom_str, "reason": reason}
                        )
                        ...
                        try:
                            ...
                            if repaired.is_valid:
                                ...
                                if format_type == "WKT":
                                    ...
                                    df.at[i, geom_column] = json.dumps(
                                        shapely.geometry.mapping(repaired)
                                    )
                        except Exception as e:
                            logger.warning(
                                f"Failed to repair geometry at index {i}: {e}"
                            )
                if bounds and not geom.is_empty:
                    geom_bounds = geom.bounds
                    if (
                        geom_bounds[0] < bounds[0]
                        or geom_bounds[2] > bounds[2]
                        or geom_bounds[1] < bounds[1]
                        or geom_bounds[3] > bounds[3]
                    ):
                ...
                if len(validation_results["invalid_samples"]) < 10:
                    validation_results["invalid_samples"].append(
                        {"index": i, "geometry": geom_str, "error": str(e)}
                    )
        validity_percentage = (
            (validation_results["valid_count"] / validation_results["total_geometries"])
            if validation_results["total_geometries"] > 0
            else 0
        )
        validation_results["validity_percentage"] = validity_percentage * 100
        validation_results["passed"] = (
            validity_percentage >= self.validation_options["min_valid_geometries"]
        )
        if self.validation_options["generate_reports"]:
    ...
    def _generate_statistical_report(
        self, df, validation_results, expected_correlations
    ):
        report_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join(
            self.validation_options["report_dir"], f"stat_report_{report_time}.pdf"
        )
        ...
        if len(numeric_df.columns) >= 2:
            ...
            plt.xticks(
                range(len(corr_matrix.columns)), corr_matrix.columns, rotation=90
            )
    ...
    def validate_h3_cells(
        ...
        for i, h3_cell in enumerate(df[h3_column]):
            try:
                ...
                if len(validation_results["invalid_samples"]) < 10:
                    validation_results["invalid_samples"].append(
                        {"index": i, "cell": h3_cell, "error": str(e)}
                    )

        # Calculate validity percentage
        validity_percentage = (
            (validation_results["valid_count"] / validation_results["total_cells"])
            if validation_results["total_cells"] > 0
            else 0
        )
        validation_results["validity_percentage"] = validity_percentage * 100
        ...
        validation_results["passed"] = (
            validity_percentage >= self.validation_options["min_valid_geometries"]
            and validation_results["resolution_mismatches"] == 0
        )
    ...
    def _generate_geometry_report(self, df, validation_results, format_type):
        ...
        report_time = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join(
            self.validation_options["report_dir"], f"geom_report_{report_time}.json"
        )
        report = {
    ...
    def validate_dataset(
        ...
        if is_h3:
            ...
            geom_results = self.validate_geometries(
                validate_df, geom_column, format_type
            )
        ...
        logger.info("\n=== Validation Summary ===")
        logger.info(
            f"Overall validation: {'PASSED' if results['passed'] else 'FAILED'}"
        )
        logger.info(
            f"Statistical validation: {'PASSED' if stat_results['passed'] else 'FAILED'}"
        )
        logger.info(f"- Correlation issues: {len(stat_results['correlation_issues'])}")
        logger.info(
            f"- Distribution issues: {len(stat_results['distribution_issues'])}"
        )
        ...
        if is_h3:
            logger.info(
                f"H3 validation: {'PASSED' if geom_results['passed'] else 'FAILED'}"
            )
            logger.info(
                f"- Valid H3 cells: {geom_results['valid_count']:,} ({geom_results['validity_percentage']:.2f}%)"
            )
            logger.info(f"- Invalid H3 cells: {geom_results['invalid_count']:,}")
            logger.info(
                f"- Resolution mismatches: {geom_results['resolution_mismatches']:,}"
            )
            logger.info(f"- Out of bounds: {geom_results['out_of_bounds_count']:,}")
            if land_geometry:
                logger.info(
                    f"- Out of land boundaries: {geom_results['out_of_land_count']:,}"
                )
        else:
            logger.info(
                f"Geometry validation: {'PASSED' if geom_results['passed'] else 'FAILED'}"
            )
            logger.info(
                f"- Valid geometries: {geom_results['valid_count']:,} ({geom_results['validity_percentage']:.2f}%)"
            )
...
def validate_generated_data(
    ...
    if (
        "repaired_count" in results["geometry_validation"]
        and results["geometry_validation"]["repaired_count"] > 0
    ):
        logger.info(
            f"Repaired {results['geometry_validation']['repaired_count']} geometries"
        )
    return df, results
    return df, results
...
if __name__ == "__main__":
    ...
    if args.config:
        ...
        if geometry_type == 4:
            if not H3_AVAILABLE:
                logger.error(
                    "Error: H3 library is not installed. Please install it using:"
                )
        ...
        format_choice = validate_input(
            "Choose format: 1 for WKT, 2 for GeoJSON\nEnter choice: ", [1, 2], int
        )
        include_demographic = (
            validate_input(
                "Include demographic columns (Gender, Occupation, Education Level)? (yes/no): ",
                ["yes", "no"],
            ).lower()
            == "yes"
        )
        include_economic = (
            validate_input(
                "Include economic columns (Household Income, Employment Status, Access to Healthcare)? (yes/no): ",
                ["yes", "no"],
            ).lower()
            == "yes"
        )
        ...
        num_columns = validate_input(
            f"Enter number of columns (min {min_cols}, max {max_cols}): ",
            range(min_cols, max_cols + 1),
            int,
        )
        use_spatial_clustering = (
            validate_input(
                "Use spatial clustering to create realistic population distributions? (yes/no): ",
                ["yes", "no"],
            ).lower()
            == "yes"
        )
        area_choice = validate_input(
            "Choose area: 1 for Jakarta, 2 for Yogyakarta, 3 for Indonesia, 4 for Japan, 5 for Vietnam\nEnter choice: ",
            [1, 2, 3, 4, 5],
            int,
        )
        geojson_path = ""
        if area_choice in [1, 3, 4, 5]:
            geojson_path = validate_input(
                f"Enter path to GeoJSON file for {BOUNDING_BOXES[area_choice]['name']} land boundaries (e.g., geojson/jkt.geojson): ",
                None,
                str,
                allow_empty=True,
            )
        ...
        if num_rows > chunking_threshold:
            use_chunking = (
                validate_input(
                    f"Large dataset detected ({num_rows} rows). Use chunked file output? (yes/no, default: yes): ",
                    ["yes", "no", ""],
                    str,
                ).lower()
                != "no"
        ...
        if H3_AVAILABLE:
            geometry_type = validate_input(
                "Choose geometry type: 1 for POINT, 2 for POLYGON, 3 for MULTIPOLYGON, 4 for H3\nEnter choice: ",
                [1, 2, 3, 4],
                int,
            )
        else:
            geometry_type = validate_input(
                "Choose geometry type: 1 for POINT, 2 for POLYGON, 3 for MULTIPOLYGON\nEnter choice: ",
                [1, 2, 3],
                int,
            )
    ...
    logger.info(
        f"Generating {num_rows:,} rows of {geom_type} geometries for {area_name}..."
    )
    land_geometry = None
    if geojson_path:
    ...
    if geom_type == "H3":
        filename_prefix = (
            f"{area_name.lower()}_data_{num_rows}r_{num_columns}c_h3_res{h3_resolution}"
        )
    else:
    ...
    logger.info(
        f"Data generation completed in {elapsed_time.total_seconds():.2f} seconds"
    )
    if use_chunking:
